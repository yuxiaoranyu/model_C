...
bool YoloResnetInference::load(const std::string &param)
{
    ...
    {
        qInfo() << "YoloResnetInference can not construct transform-coordinate";
        return false;
    }
...
std::vector<BaseAIResult> YoloResnetInference::infer(const std::vector<int> &ids, const std::vector<cv::Mat> &images)
{
    ...
    if (ids.size() > 1) {
        qInfo() << "YoloResnetInference error, input image size is: " << ids.size();
    ...
    if (yoloparam.yolo_model->infer(images, yoloResult))
    {
        ...
        {
            if (yoloResult[yolo_index].size() != 0)  //仅分析检测出的第一个目标
            {
                int x1 = yoloResult[yolo_index][0].box.tl().x;
                int y1 = yoloResult[yolo_index][0].box.tl().y;
                int x2 = yoloResult[yolo_index][0].box.br().x;
                int y2 = yoloResult[yolo_index][0].box.br().y;
                ...
                if (resnetparam.resnet_model->infer(resnet_input, resnetResult))
                {
                    int classId = resnetResult[yolo_index].classId; //检测目标的分类ID
                    ...
                    if (isDefective) //杂物类别
                    {
                        ...
                        pos_infoObj.insert("id", int(yolo_index)); //pos_info的id，从0开始编号
                        pos_infoObj.insert("moduleId", 1);  //yolo为0，resnet为1
