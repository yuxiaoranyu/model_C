...
namespace Yolo
{
    ...
    static constexpr float IGNORE_THRESH = 0.1f; //忽略框的置信度
    struct YoloKernel
    {
        int width;
        int height;
    ...
    static constexpr int MAX_OUTPUT_BBOX_COUNT = 1000; //最多可预测的框数目
    static constexpr int CLASS_NUM = 1;  // 类别数
    static constexpr int INPUT_H = 640;  // 网络处理高度 yolov5's input height and width must be divisible by 32.
    static constexpr int INPUT_W = 640;  // 网络处理宽度

    static constexpr int LOCATIONS = 4; //4个坐标值
    struct alignas(float) Detection {
        //center_x center_y w h
        float bbox[LOCATIONS];  //框坐标值
